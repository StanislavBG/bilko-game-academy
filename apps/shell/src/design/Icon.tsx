...
export type IconName =
  | 'home' | 'compass' | 'trophy' | 'user' | 'gear' | 'gem' | 'coin' | 'map'
  | 'play' | 'lock' | 'check' | 'arrow-right' | 'arrow-left' | 'flag'
  | 'skull' | 'fire' | 'flame' | 'shield' | 'wind' | 'star' | 'clover'
  | 'dice' | 'bolt' | 'wave' | 'volume' | 'eye' | 'battery' | 'anchor'
  | 'sparkle' | 'book' | 'rocket' | 'plus' | 'pencil' | 'dot';
...
/** Inline icon set — geometric, friendly, no emoji. Ported verbatim from
 *  the design handoff so visual weight matches the prototype. */
export function Icon({ name, size = 20, stroke = 1.75, className, style }: IconProps): JSX.Element {
  const common = {
    width: size,
    height: size,
    viewBox: '0 0 24 24',
